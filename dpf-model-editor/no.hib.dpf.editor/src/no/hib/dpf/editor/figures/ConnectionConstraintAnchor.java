...
public class ConnectionConstraintAnchor implements ConnectionAnchor {
	 ...
	 * 
	 * @param p the point where this anchor will be located (unused)
	 * @param useTargetEnd instructs the anchor to anchor from the target
	...
	/**
	 * Returns the location of this anchor. The reference point is ignored in this implementation.
	...
	private Point getLinePoint() {
		...
		throw new RuntimeException("ConnectionConstraintAnchor: No connectionFigure set.");
	}
	...
	private Point calculateConnectionPoint(PointList points, boolean fromFirstPoint) {
		Point endPoint = getEndPoint(points, fromFirstPoint);
		if (points.size() < 2) return endPoint;
	...
	private double findDistanceToConnectionPoint(double distanceBetweenFirstPoints) {
	...
	private Point calculateConnectionPoint(Point endPoint, Point secondPoint, double distanceBetweenFirstPoints, double distanceToConnectionPoint) {
