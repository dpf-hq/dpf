 ...
 *******************************************************************************/
package no.hib.dpf.editor.parts.arrow;
...
import no.hib.dpf.editor.policies.arrow.ArrowBendpointEditPolicy;
...
public class DArrowEditPart extends GraphicalConnectionEditPart implements NodeEditPart {
	...
	protected void createEditPolicies() {
		...
		// used for adding arrowattributes to the arrow
		...
		if (arrowPaint == null)
			try {
				...
				IConfigurationElement configure = FigureConfigureManager
						.getInstance().getConfigurationElement(name);
				if (configure != null)
					arrowPaint = (IArrowPainting) configure
	...
	protected void setArrowHead(PolylineConnection connectionFigure) {
		...
		connectionFigure.setTargetDecoration(arrowHead); // arrow at target
															// endpoint
	}
	...
	PropertyChangeListener listener = new PropertyChangeListener() {
		...
		public void propertyChange(PropertyChangeEvent evt) {
			if (Connection.PROPERTY_CONNECTION_ROUTER.equals(evt
					.getPropertyName())) {
				refreshBendpoints();
				refreshBendpointEditPolicy();
			}
	...
	// points.getLastPoint().getDistance(points.getPoint(points.size() - 2));
	...
	//
	// int deltaX = points.getPoint(points.size() - 2).x -
	// points.getPoint(points.size() - 1).x;
	// int deltaY = points.getPoint(points.size() - 2).y -
	// points.getPoint(points.size() - 1).y;
	//
	// deltaX = (int)(deltaX * factor);
	// deltaY = (int)(deltaY * factor);
	//
	// return new Point(points.getLastPoint().x + deltaX,
	// points.getLastPoint().y + deltaY - 10);
	// }
	//
	// }
	...
	protected void refreshBendpoints() {
		...
		if (getSource() != null && getTarget() != null) {
			Rectangle source = ((NodeEditPart) getSource()).getFigure()
					.getBounds();
			Rectangle target = ((NodeEditPart) getTarget()).getFigure()
					.getBounds();
	...
	protected List<?> getModelSourceConnections() {
		...
		sources.addAll(getDArrow().getDOutgoingAttributes());
		return sources;
	}
