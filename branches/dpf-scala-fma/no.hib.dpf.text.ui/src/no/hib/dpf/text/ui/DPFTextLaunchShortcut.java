	...
	protected ILaunchConfiguration findLaunchConfiguration(IFile file, ILaunchConfigurationType configType) {
		...
		try {
			...
			for (int i = 0; i < configs.length; i++) {
				...
				if (config.getAttribute(DPFTextLauncher.LAUNCH_FILE, "").equals(file.getName())) { //$NON-NLS-1$
    ...
    protected ILaunchConfiguration createConfiguration(IFile file) {
       ILaunchConfiguration config = null;
        try {
            ...
            wc.setAttribute(DPFTextLauncher.LAUNCH_FILE, file.getFullPath().toPortableString());
            config = wc.doSave();           
		...
		try {
			if(selection instanceof StructuredSelection){
				...
				if(ss.getFirstElement() instanceof IFile){
				    ...
				    if(f.getName().endsWith(DPFTextLauncher.LAUNCH_FILE_ENDING)){
				    	return f;
				    }else{
				    	return null;
				    }
