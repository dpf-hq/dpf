 ...
 */
public DiagramTreeEditPart(DGraph model) {
	super(model);
}
...
protected List<DElement> getModelChildren() {
	ArrayList<DElement> list = new ArrayList<DElement>();
	list.add((DElement)getCastedModel().getDGraph().getDNodes());
	list.add((DElement)getCastedModel().getDGraph().getDDataNodes());
