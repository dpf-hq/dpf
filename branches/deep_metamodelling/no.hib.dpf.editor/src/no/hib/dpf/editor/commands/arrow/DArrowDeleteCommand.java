...
public class DArrowDeleteCommand extends CompoundCommand {
	...
	public DArrowDeleteCommand(DArrow conn) {
		...
		for(DArrowDAttribute arrowAttribute : conn.getDOutgoingAttributes()){
			if(!elements.contains(arrowAttribute)){
				add(new DArrowDAttributeDeleteCommand(arrowAttribute));
				elements.add(arrowAttribute);
			}
		}
