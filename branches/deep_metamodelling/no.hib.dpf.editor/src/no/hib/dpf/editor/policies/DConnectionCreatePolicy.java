...
import no.hib.dpf.editor.commands.arrow.DArrowCreateCommand;
import no.hib.dpf.editor.commands.arrow.DArrowReconnectCommand;
...
public class DConnectionCreatePolicy extends GraphicalNodeEditPolicy {
	...
	protected Command getConnectionCreateCommand( CreateConnectionRequest request) {
		...
		else if (objectType == DArrow.class) {
	...
	protected Command getReconnectSourceCommand( ReconnectRequest request) {
		...
		else if(model instanceof DArrow){
	...
	protected Command getReconnectTargetCommand( ReconnectRequest request) {
		...
		if(model instanceof DNodeDAttribute){
			...
			if(newTarget instanceof DDataNode){
				DNodeAttributeReconnectCommand cmd = new DNodeAttributeReconnectCommand(conn);
				cmd.setNewTarget((DDataNode)newTarget);
				return cmd;
			}
		}
			...
			DArrow conn = (DArrow) model;
			Object newTarget = getHost().getModel();
			if(newTarget instanceof DNode){
				DArrowReconnectCommand cmd = new DArrowReconnectCommand(conn);
				cmd.setNewTarget((DNode)newTarget);
				return cmd;
			}
		}
		return null;
