...
public class ExtendItemPropertyDescriptor extends ItemPropertyDescriptor {
	...
	public EditingDomain getEditingDomain(Object object)
	  ...
	  private boolean isModelElement(){
		  if(category == null)
			  return true;
		  if(this.category.equals(DPFCoreConstants.DNODE_CATEGORY) || 
				  category.equals(DPFCoreConstants.DARROW_CATEGORY) || 
				  category.equals(DConstraintItemProvider.DConstraint_CATEGORY)|| 
				  category.equals(DPFCoreConstants.DGRAPH_CATEGORY)|| 
				  category.equals(DPFCoreConstants.DARROW_DATTRIBUTE_CATEGORY) || 
				  category.equals(DPFCoreConstants.DNODE_DATTRIBUTE_CATEGORY)|| 
				  category.equals(DPFCoreConstants.DDATANODE_CATEGORY))
			  return false;
		  return true;
	  }
	  ...
	  private Object getModelElement(Object object){
		  ...
		  if(object instanceof DNodeDAttribute)
			  return ((DNodeDAttribute)object).getNodeAttribute();
		  if(object instanceof DDataNode)
			  return ((DDataNode)object).getDatanode();
		  if(object instanceof DArrow)
			  return ((DArrow)object).getArrow();
		  if(object instanceof DArrowDAttribute)
			  return ((DArrowDAttribute)object).getArrowAttribute();
		  if(object instanceof DGraph)
			  return ((DGraph)object).getGraph();
