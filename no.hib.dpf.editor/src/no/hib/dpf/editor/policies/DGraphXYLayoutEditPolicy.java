...
public class DGraphXYLayoutEditPolicy extends XYLayoutEditPolicy {
	...
	protected Command createChangeConstraintCommand(ChangeBoundsRequest request, EditPart child, Object constraint) {
		...
		if (constraint instanceof Rectangle) {
			...
			boolean move = RequestConstants.REQ_MOVE.equals(type) || RequestConstants.REQ_ALIGN_CHILDREN.equals(type)
					|| RequestConstants.REQ_MOVE_CHILDREN.equals(type);
			boolean size = RequestConstants.REQ_RESIZE.equals(type)
			...
			if (move && (child instanceof DComposedNodePart || child instanceof DNodeEditPart)) {
				...
				// newLocation =
				// newLocation.translate(compose.getBound().getCenter().getNegated());
				// }
				return new DNodeMoveCommand((DNode) child.getModel(), newLocation);
			}
			// else if(move && (child instanceof DDataNodeEditPart)){
			// Point newLocation = ((Rectangle)constraint).getLocation();
			// return new DDataNodeMoveCommand((DDataNode)child.getModel(),
			// newLocation);
			// }
			...
			if (size && child instanceof DNodeEditPart) {
				...
				// return new
				// DDataNodeResizeCommand((DDataNode)child.getModel(),
				// ((Rectangle)constraint).getSize());
			}
		}
	...
	protected Command getCreateCommand(CreateRequest request) {
		...
		if (childClass == DNode.class) {
			...
			// // return a command that can add a Shape to a DPFDiagram
			// return new DDataNodeCreateCommand(ddatanode,
			// ((DGraph)getHost().getModel()));
		}
		return null;
	}
	...
	// public List<DDataNodeEditPart> getDDataNodeEditParts(DGraphEditPart
	// parent, DDataNode type){
	...
	// if(dDatanode.getDType() == type){ // TODO needs to be improved for
	// different subtypes?
	...
	// private List<DArrowEditPart> getDArrowEditParts(DGraphEditPart parent,
	// DArrow type) {
