...
public class DNodeEditPart extends GraphicalEditPartWithListener implements NodeEditPart{
	...
	protected INodePainting nodePaint;
	...
	protected List<?> getModelSourceConnections() {
		...
		sources.addAll(getDiagramModel().getDOutgoingAttributes());
	...
	protected List<?> getModelTargetConnections() {
		...
		targets.addAll(getDiagramModel().getConstraintsTo());
		return targets;
	...
	protected INodePainting getNodePaint() {
		if(nodePaint == null)
			try {
				String name = null;
				if(getDiagramModel() instanceof DNode)
					name = getDiagramModel().getDType().getConfigureString();
			...
			case DiagramPackage.DNODE__SIZE:
				...
				for(Object connect : getTargetConnections()){
					if(connect instanceof DConstraintEditPart && ((DConstraintEditPart)connect).getSource() instanceof DComposedNodePart){
						...
						editPart.resize(getDiagramModel().getLocation());
	...
	protected void refreshVisuals() {
		...
		Point loc = getDiagramModel().getLocation();
		if(loc == null){
			loc = new Point(5, 5);
		}
		
		Dimension size = getDiagramModel().getSize();
		if(size == null){
