...
sig EBarcode6{src:one NScanNurseCard, trg:one NGetNurseInfo}
...
sig ESample8{src:one NSendSample, trg:one NSendSample1}
...
sig EBloodhavenotbeenordered5{src:one NSendSample, trg:one NOrderBlood}
...
sig Eouttimelessthan4h7{src:one NPutBloodIntorefrigertor, trg:one NScanBloodProduct}
...
sig EBarcode4{src:one NSelectBloodProduct, trg:one NGetDoctorInfo}
...
sig EBarcode2{src:one NScanSample, trg:one NGetSampleInfo}
...
sig EBarcode3{src:one NScannPatientWristband, trg:one NGetPatientInfo0}
...
//facts
fact xor3surj_Eouttimelessthan4h_EMoreblood_Eouttimelessthan4h7{
	all x:NScanBloodProduct | let e1 = (some e : Eouttimelessthan4h7 | e.trg = x), 
...
fact iff_Eouttimelessthen05h_Eouttimelessthan4h7{
	all y:NPutBloodIntorefrigertor | (some yz:Eouttimelessthan4h7| yz.src=y) iff (some xy:Eouttimelessthen05h| xy.trg = y)
}
...
fact special_Eouttimelessthan4h_Eouttimelessthan4h7{
	some NScanBloodProduct implies ((some z:NScanBloodProduct|some xz:Eouttimelessthan4h|xz.trg=z) or (some z:NScanBloodProduct|some yz:Eouttimelessthan4h7|yz.trg=z))
}
...
fact mult1_Eouttimelessthan4h7{
	//mulitplicity on out time less than 4h:Put Blood Into refrigertor->Scan Blood Product[0,1]
	all n:(NPutBloodIntorefrigertor)|lone e:(Eouttimelessthan4h7)|e.src=n
}
...
fact mult1_ESample8{
	//mulitplicity on (Sample*):Send Sample->Send Sample[0,1]
	all n:(NSendSample)|lone e:(ESample8)|e.src=n
}
...
fact mult1_EBloodhavenotbeenordered5{
	//mulitplicity on Blood have not been ordered:Send Sample->Order Blood[0,1]
	all n:(NSendSample)|lone e:(EBloodhavenotbeenordered5)|e.src=n
}
...
fact surj_EBarcode4{
	...
	all n:(NGetDoctorInfo)| some e:(EBarcode4)| e.trg = n
}
...
fact surj_EBarcode6{
	...
	all n:(NGetNurseInfo)| some e:(EBarcode6)| e.trg = n
}
...
fact surj_EBarcode3{
	...
	all n:(NGetPatientInfo0)| some e:(EBarcode3)| e.trg = n
}

fact surj_EBarcode2{
	//surjective on (Barcode):Scan Sample->Get Sample Info
	all n:(NGetSampleInfo)| some e:(EBarcode2)| e.trg = n
}
...
fact multi1_NScanNurseCard{
	...
	let count = #NScanNurseCard | count>=1 and count <=1}
...
fact imply2_EBloodhavenotbeenordered5_Ebloodtypeisunknownorscreeningisnotdoneorinvalidscreenisvalidwithin4days{
	all y1:NOrderBlood, y2:NOrderSample | let e1 = (some e : EBloodhavenotbeenordered5 | e.trg = y1), 
...
fact xor_ENSendSampleNCollectBlood_EBloodhavenotbeenordered5{
	//XOR constraint between :Send Sample->Collect Blood and Blood have not been ordered:Send Sample->Order Blood
	all n:(NSendSample) | let e1 = (some e : ENSendSampleNCollectBlood | e.src = n), e2=(some e : EBloodhavenotbeenordered5 | e.src = n)|(e1 or e2) and not(e1 and e2)
}
...
fact surj_ESample8{
	//surjective on (Sample*):Send Sample->Send Sample
	all n:(NSendSample1)| some e:(ESample8)| e.trg = n
}
...
fact xor3surj_EBloodhavenotbeenordered_EBloodhavenotbeenordered5_ENShowPatientInfoNOrderBlood{
	all x:NOrderBlood | let e1 = (some e : ENShowPatientInfoNOrderBlood | e.trg = x), 
e2 = (some e : EBloodhavenotbeenordered5 | e.trg = x), 
...
fact surj_ENShowNurseInfoNScanPatientWristband{
	//surjective on :Show Nurse Info->Scan Patient Wristband
	all n:(NScanPatientWristband)| some e:(ENShowNurseInfoNScanPatientWristband)| e.trg = n
}
...
fact iff_EPatienthasnotbeenbloodtyped_ENSelectBlodTypeingNSendSampleOrder{
	all y:NSelectBlodTypeing | (some yz:ENSelectBlodTypeingNSendSampleOrder| yz.src=y) iff (some xy:EPatienthasnotbeenbloodtyped| xy.trg = y)
}
...
fact mult1_EBarcode2{
	...
	all n:(NScanSample)|lone e:(EBarcode2)|e.src=n
}

fact mult1_EBarcode4{
	//mulitplicity on (Barcode):Select Blood Product->Get Doctor Info[0,1]
	all n:(NSelectBloodProduct)|lone e:(EBarcode4)|e.src=n
}

fact mult1_EBarcode3{
	//mulitplicity on (Barcode):Scann Patient Wristband->Get Patient Info[0,1]
	all n:(NScannPatientWristband)|lone e:(EBarcode3)|e.src=n
}
...
fact mult1_EBarcode6{
	//mulitplicity on (Barcode):Scan Nurse Card->Get Nurse Info[0,1]
	all n:(NScanNurseCard)|lone e:(EBarcode6)|e.src=n
}
