 	...
 	def validate(is:IS){
		...
		for(e<-createValidationParts(is)){
			...
			try{
			  ...
			  case ex => errors.add("Error validate(is:IS): " + ex.getMessage())
			}			
 	  }
    	 ...
    	 try{
    		 if(context==e.eClass()){
    		     if(!ocl.check(e, constraint)){
    		    	 val validateId = e.eGet(e.eClass().getEStructuralFeature("id"));
    			     val message = "OCL=" +  constraint + " is not valid!";
    			     errors.add(RId(validateId.toString.toLong),message)
    		     }
     		 }
    	 }catch{
    	   case ex => errors.add("validate1(modelResource: EObject,oclString: String): " + ex.getMessage());
    	 }
	  }
   }   
