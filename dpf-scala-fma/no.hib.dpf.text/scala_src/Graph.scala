...
case class SignatureConstraint(id:Id,n:String,ps:List[String])
...
case class S(g:AbstractGraph,cs:Set[Constraint]){  //Specification
	...
	def format(c:Constraint):String=c.s.n + "(" +c.s.ps + ")"+ "{Arrows(" + arrowsToString(c.as) + ")}";
