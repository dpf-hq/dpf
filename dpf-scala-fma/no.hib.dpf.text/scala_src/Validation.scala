...
trait Helper{
   ...
   /**
    * Container for error messages that occured through validation
    */
   object errors{
  ...
  def fillOcl(c:Constraint,v:Validator)={
		  ...
		  case OclPe(e)		=> v.nodeVars.get(e) match {
		    					 ...
		    					 case None 		=>
			    					v.arrowVars.get(e) match {
			    					 ...
			    					 case None 		=> errors.add("Variable " + e + " is not specified!")
			  					   }   
		  					   }
   ...
   private def createShape(arrows:List[Arrow]):List[Arrow]={
	 ...
	 val r = for(a<-arrows)yield{
	     ...
	     case _ => sys.error("Programming Error in createShape()")
	     }	
	 }
//     println(arrows)
//     println(r)
   ...
   def validateShapes(is:IS)={
     for(c<-is.mm.cs){
		  ...
		  def printError =  errors.add("Shapes do not fit to" + shapeConstraint + " <> " + shapeValidator)
		  ...
		  if(shapeConstraint.length==shapeValidator.length){
		    for(i <-0 to shapeConstraint.length-1){
		    	...
		    	shapeValidator(i) match {
		    	  ...
		    	  case _ => sys.error("Programming Error in validateShapes(is:IS)")
		    	}
		    }
