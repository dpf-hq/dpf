...
public class AbstractDPFTextSemanticSequencer extends AbstractSemanticSequencer {
	...
	public void createSequence(EObject context, EObject semanticObject) {
		if(semanticObject.eClass().getEPackage() == TdpfPackage.eINSTANCE) switch(semanticObject.eClass().getClassifierID()) {
			...
			case TdpfPackage.SEQUENCE_NUMBER:
				if(context == grammarAccess.getSequenceNumberRule()) {
					sequence_SequenceNumber(context, (SequenceNumber) semanticObject); 
					return; 
				}
				else break;
			case TdpfPackage.SIGNATURE:
				if(context == grammarAccess.getModelRule() ||
				   context == grammarAccess.getSignatureRule()) {
	...
	/**
	 * Constraint:
	 *     (id=DpfId (parameter+=STRING parameter+=STRING*)?)
	 *
	 * Features:
	 *    id[1, 1]
	 *    parameter[0, *]
	...
	/**
	 * Constraint:
	 *     (id=ID extendsGraph=ChoosenExtSubTGraph (elements+=Element elements+=Element*)?)
	 *
	 * Features:
	 *    id[1, 1]
	 *    extendsGraph[1, 1]
	 ...
	 *    nextNumber[1, 1]
	 */
	protected void sequence_SequenceNumber(EObject context, SequenceNumber semanticObject) {
		if(errorAcceptor != null) {
			if(transientValues.isValueTransient(semanticObject, TdpfPackage.Literals.SEQUENCE_NUMBER__NEXT_NUMBER) == ValueTransient.YES)
				errorAcceptor.accept(diagnosticProvider.createFeatureValueMissing(semanticObject, TdpfPackage.Literals.SEQUENCE_NUMBER__NEXT_NUMBER));
		}
		INodesForEObjectProvider nodes = createNodeProvider(semanticObject);
		SequenceFeeder feeder = createSequencerFeeder(semanticObject, nodes);
		feeder.accept(grammarAccess.getSequenceNumberAccess().getNextNumberINTTerminalRuleCall_0(), semanticObject.getNextNumber());
		feeder.finish();
	}
	 ...
	 * Constraint:
	 *     (
	 *         (type=ChoosenSpecification | (type=ChoosenSpecification constraintSemantic=ChoosenConstraintSemantic)) 
