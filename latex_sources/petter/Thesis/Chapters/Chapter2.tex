...
models at higher levels of abstraction and producing models that contains a
...
software developers and customers. Because a customer is usually not an expert
...
process then this could help to narrow the gap in communication between
developers and stakeholders. Because now we can apply model transformation that changes input
...
importance of OMG in for the software industry. MDA is build around many
...
describes parts of the application. In MDA these diagrams or models are the
main artifact for the corresponding phases, instead of just a reference for
developers to use when implementing code. The architecture then generates
implementation code based on these models. A traditional software development
process would have iterations for the implementation and testing to make sure
that the application meets the demands of the customer. This process is
continued for every iteration, where developers continually use the text and
...
meta-modeling language. Models at the M\textsubscript{2} layer represents the
abstract syntax for models created in the M\textsubscript{1} layer. This
...
every modeling element on every layer corresponds to some modeling element of
one layer higher. One could say that MOF is a Domain Specific Language (DSL) to
create meta-models. 
...
associations, etc. A constraint is a restriction on how these elements should
...
to designing the MOF 2.0 version of the standard that was named Essential
EMOF (EMOF). EMF provides the meta-model Ecore that is aligned to EMOF and is a
modeling language to build modeling languages. Ecore is essentially a simplified
...
Because in DPF a modeling language is represented as a modeling formalism. A
modeling formalism in DPF is defined by a set of atomic constraints and a
specification that has an underlying graph and a set of atomic constraints. For
example figure~\ref{fig:modeling_formlalism} represents three levels of
abstractions for defining a DSML. The specification $\spec{S}$\textsubscript{1}
that is defined at the first layer corresponds to the modeling formalism one
layer higher. The modeling formalism consist of a specification
$\spec{S}$\textsubscript{2} that has an underlying graph S\textsubscript{2} and
a set of predicates Z\textsubscript{3}. Together with specification
$\spec{S}$\textsubscript{2} a set of predicates Z\textsubscript{2} can be
defined together with the underlying graph S\textsubscript{2} to define a
...
Z\textsubscript{2} provides the semantics while the
specification $\spec{S}$\textsubscript{2} provides the abstract syntax for
defining a new specification $\spec{S}$\textsubscript{1}.
Figure~\ref{fig:MOF_vs_DPF} explains the difference between OMG's MOF on the
left side and DPF's multi layer meta-modeling hierarchy on the right side
...
environments provides two abstraction layers, DPF on the other hand provides a
possible unlimited abstraction layers users can interact with. The reason that
MOF based modeling environments provides two layers to interact with is because
the layers M\textsubscript{2} and M\textsubscript{3} are usually part of the
...
DPF are unable to interact with located at the highest abstraction layer. A
...
generates a new graphical editor environment provided by the DPF Editor.
...
support for creating domain specific modeling languages. Figure~\ref{fig:DPF_Editor} is a
simplified version provided in the article, \textit{``DPF Workbench: a
multi-level language workbench for MDE"}\cite{Lamo2013} .

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.7]{./Figures/DPF_Editor}
...
A new DPF Editor is generated for each new model that is created. This means
that every DPF model has a corresponding editor that provides graphical editing
properties to change the models. For each generated editor we can create a new
DSL one abstraction layer lower that generates a new editor that
correspond to this DSL. 
