...
private boolean checkSourceReconnection() {
	...
	for (Iterator<DArrow> iter = newSource.getDOutgoings().iterator(); iter.hasNext();) {
		...
		if (conn.getDTarget().equals(oldTarget) &&  !conn.equals(connection)) {
			return false;
		}
	}
	return true;
...
private boolean checkTargetReconnection() {
	...
	// return false, if the connection exists already
	for (Iterator<DArrow> iter = newTarget.getDIncomings().iterator(); iter.hasNext();) {
		DArrow conn = iter.next();
		// return false if a oldSource -> newTarget connection exists already
		// and it is a differenct instance that the connection-field
		if (conn.getDSource().equals(oldSource) && !conn.equals(connection)) {
			return false;
		}
	}
	return true;
